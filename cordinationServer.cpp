	...
	while (1)
	{
		...
		if (BufferSize != 0)
		{
			...
			else if (strcmp(document["request_type"].GetString(), "client_put_request") == 0)
			{
				...
				int suc = slave_id;
				Node *pre = NULL, *succ = NULL;
				//we will find successor of hashed number from tree
			...
			else if (strcmp(document["request_type"].GetString(), "client_get_request") == 0)
			{
				...
				else
				{
					...
					//if server is down, we have to go to its successor, as it would have copied our data to it.
					else
					{
						Node *pre = NULL, *succ = NULL;
...
int main(int argc, char const *argv[])
{
	...
	while (1)
	{
		...
		else if (strcmp(document["request_type"].GetString(), "register_client") == 0)
		{
			...
			//pthread_detach means that, now this thread will run independently without exsistence of main thread.
		...
		else if (strcmp(document["request_type"].GetString(), "register_slave") == 0)
		{
			...
			cout << "Parsing of the document for slave registeration is successful" << endl;
